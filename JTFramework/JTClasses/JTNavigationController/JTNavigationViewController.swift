...
open class JTNavigationViewController: UIViewController {
    ...
    public weak var optionalDelegate: JTNavigationViewControllerOptionalDelegate?
    public weak var delegate: JTNavigationViewControllerDelegate?
    
    public init(_ height: CGFloat = 50) {
        var x = height
        if x < 50 { x = 50 }
...
extension JTNavigationViewController {
    ...
    private func setupViewLayer() {
        let layer = CALayer()
        layer.frame = CGRect(x: 0, y: navigationViewHeight, width: Global_Common.shareInstance.ScreenWidth, height: 1)
...
public protocol JTNavigationViewControllerOptionalDelegate: NSObjectProtocol {
    func backTouchUpInsideRecognizeJTNavigation() -> Bool
}
public protocol JTNavigationViewControllerDelegate: NSObjectProtocol {
